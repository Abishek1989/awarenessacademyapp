...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!authData || !authData.user || authData.user.isDefaultAdmin !== true) {
    if (typeof UI !== "undefined" && UI.createPopup) {
      ...
      alert(
        "Access Denied. Only the Default Administrator can view this page.",
      );
...
async function fetchRateLimits() {
  try {
    ...
    if (res.ok) {
      ...
      document.getElementById("rateMaxHits").textContent =
        d.maxHits + " / 15m window";
    }
  } catch (e) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      data.forEach((user) => {
        ...
        const roleBadge =
          user.role === "Admin"
            ? `<span style="background: rgba(245, 158, 11, 0.2); color: #f59e0b; padding: 2px 6px; border-radius: 4px; font-size: 0.75rem;">Admin</span>`
            : user.role === "Staff"
              ? `<span style="background: rgba(59, 130, 246, 0.2); color: #3b82f6; padding: 2px 6px; border-radius: 4px; font-size: 0.75rem;">Staff</span>`
...
async function loadSettingsAndCostMap() {
  try {
    ...
    if (res.ok) {
      ...
      document.getElementById("vpsInstances").value =
        devSettings.vpsInstances || 1;

      document.getElementById("mongoCost").value = devSettings.mongoCost || 0;
      document.getElementById("mongoStoragePrice").value =
        devSettings.mongoStoragePricePerGb || 0.25;
      document.getElementById("mongoDataTransferPrice").value =
        devSettings.mongoDataTransferPricePerGb || 0.12;
      document.getElementById("mongoBackupCost").value =
        devSettings.mongoBackupCost || 0;

      document.getElementById("rzpPercent").value =
        devSettings.razorpayCommissionPercent || 2.0;

      // R2 Video System Settings
      if (document.getElementById("r2MaxVideoSize"))
        document.getElementById("r2MaxVideoSize").value =
          devSettings.r2MaxVideoSizeBytes || 5368709120;
      if (document.getElementById("r2ChunkSize"))
        document.getElementById("r2ChunkSize").value =
          devSettings.r2ChunkSizeBytes || 52428800;
      if (document.getElementById("r2ClassACount"))
        document.getElementById("r2ClassACount").value =
          devSettings.r2ClassACount || 0;
      if (document.getElementById("r2ClassAWarning1"))
        document.getElementById("r2ClassAWarning1").value =
          devSettings.r2ClassAWarning1 || 500000;
      if (document.getElementById("r2ClassAWarning2"))
        document.getElementById("r2ClassAWarning2").value =
          devSettings.r2ClassAWarning2 || 750000;
      if (document.getElementById("r2ClassAStop"))
        document.getElementById("r2ClassAStop").value =
          devSettings.r2ClassAStop || 900000;

      if (document.getElementById("r2ClassBCount"))
        document.getElementById("r2ClassBCount").value =
          devSettings.r2ClassBCount || 0;
      if (document.getElementById("r2ClassBWarning1"))
        document.getElementById("r2ClassBWarning1").value =
          devSettings.r2ClassBWarning1 || 5000000;
      if (document.getElementById("r2ClassBWarning2"))
        document.getElementById("r2ClassBWarning2").value =
          devSettings.r2ClassBWarning2 || 7500000;
      if (document.getElementById("r2ClassBStop"))
        document.getElementById("r2ClassBStop").value =
          devSettings.r2ClassBStop || 9000000;

      // Update KPIs
      if (document.getElementById("kpiClassA"))
        document.getElementById("kpiClassA").textContent =
          `${(devSettings.r2ClassACount || 0).toLocaleString()} / 1M`;
      if (document.getElementById("kpiClassB"))
        document.getElementById("kpiClassB").textContent =
          `${(devSettings.r2ClassBCount || 0).toLocaleString()} / 10M`;

      // Scaling logic
      document.getElementById("autoScaleToggle").checked =
        devSettings.autoScaleEnabled;
      document.getElementById("scaleThreshold").value =
        devSettings.scalingThresholdPercent || 80;
      document.getElementById("maxInstances").value =
        devSettings.maxInstancesAllowed || 3;
...
function calculateCostSummaries() {
  ...
  document.getElementById("calcRzpTotal").textContent =
    estCommission.toFixed(2);
  document.getElementById("calcR2Total").textContent = r2Cost.toFixed(2);

  // Update Summary Panel
  document.getElementById("sumInfraCost").textContent =
    "$" + totalInfra.toFixed(2);
  document.getElementById("sumTotalRevenue").textContent =
    "₹" + totalRevenue.toFixed(2);
  document.getElementById("sumRevenue").textContent =
    "₹" + currentRevenue.toFixed(2);
  document.getElementById("sumCommission").textContent =
    "₹" + estCommission.toFixed(2);
...
async function saveSettings() {
  try {
    const payload = {
      ...
      vpsInstances:
        parseInt(document.getElementById("vpsInstances").value) || 1,
      mongoCost: parseFloat(document.getElementById("mongoCost").value) || 0,
      mongoStoragePricePerGb:
        parseFloat(document.getElementById("mongoStoragePrice").value) || 0.25,
      mongoDataTransferPricePerGb:
        parseFloat(document.getElementById("mongoDataTransferPrice").value) ||
        0.12,
      mongoBackupCost:
        parseFloat(document.getElementById("mongoBackupCost").value) || 0,
      razorpayCommissionPercent:
        parseFloat(document.getElementById("rzpPercent").value) || 2.0,
      autoScaleEnabled: document.getElementById("autoScaleToggle").checked,
      scalingThresholdPercent:
        parseInt(document.getElementById("scaleThreshold").value) || 80,
      maxInstancesAllowed:
        parseInt(document.getElementById("maxInstances").value) || 3,
      r2MaxVideoSizeBytes:
        parseInt(document.getElementById("r2MaxVideoSize")?.value) ||
        5368709120,
      r2ChunkSizeBytes:
        parseInt(document.getElementById("r2ChunkSize")?.value) || 52428800,
      r2ClassAWarning1:
        parseInt(document.getElementById("r2ClassAWarning1")?.value) || 500000,
      r2ClassAWarning2:
        parseInt(document.getElementById("r2ClassAWarning2")?.value) || 750000,
      r2ClassAStop:
        parseInt(document.getElementById("r2ClassAStop")?.value) || 900000,
      r2ClassBWarning1:
        parseInt(document.getElementById("r2ClassBWarning1")?.value) || 5000000,
      r2ClassBWarning2:
        parseInt(document.getElementById("r2ClassBWarning2")?.value) || 7500000,
      r2ClassBStop:
        parseInt(document.getElementById("r2ClassBStop")?.value) || 9000000,
    };
    ...
    if (res.ok) {
      alert(
        "✅ Settings saved successfully! Your cost parameters have been stored.",
      );
    ...
    console.error(e);
    alert(
      "❌ Error saving settings. Please check your connection and try again.",
    );
  }
}
...
function startWebSocket() {
  ...
  connectionStatus.innerHTML =
    '<i class="fas fa-check-circle"></i> Connecting to Live Data WebSocket...';
  ...
  evtSource.onopen = () => {
    connectionStatus.className = "status-bar status-connected";
    connectionStatus.innerHTML =
      '<i class="fas fa-check-circle"></i> Connected to Live Data Stream';
    setTimeout(() => {
  ...
  evtSource.onclose = () => {
    ...
    connectionStatus.innerHTML =
      '<i class="fas fa-exclamation-circle"></i> Connection Lost. Reconnecting...';

    // Ensure we trigger the background broadcaster via standard REST endpoint as a fallback activator
    fetch(`${Auth.apiBase}/developer/metrics/stream`, {
      headers: Auth.getHeaders(),
    }).catch(() => {});
...
function checkAlertThresholds(sys) {
  ...
  if (
    sys.cpuUsagePct > 90 &&
    (!lastAlertTime["cpu"] || now - lastAlertTime["cpu"] > alertCooldown)
  ) {
    showDevAlert(
      "CRITICAL Load: CPU Overload",
      `Server CPU usage has reached ${sys.cpuUsagePct}%! Immediate attention required.`,
      true,
    );
  ...
  if (
    sys.ramUsagePct > 90 &&
    (!lastAlertTime["ram"] || now - lastAlertTime["ram"] > alertCooldown)
  ) {
    showDevAlert(
      "CRITICAL Load: RAM Exhaustion",
      `Server RAM usage is critically high at ${sys.ramUsagePct}%.`,
      true,
    );
  ...
  if (
    sys.dbStatus === "Disconnected" &&
    (!lastAlertTime["db"] || now - lastAlertTime["db"] > alertCooldown)
  ) {
    showDevAlert(
      "CRITICAL OUTAGE: Database Offline",
      `The MongoDB cluster has disconnected. Application is degraded.`,
      true,
    );
...
function processLiveData(data) {
  ...
  document.getElementById("valTotalCost").textContent =
    "$" + totalInfra.toFixed(2);
  document.getElementById("valCurrentRev").textContent =
    "₹" + currentRevenue.toFixed(2);
  ...
  if (sys.activeUsers > 0) {
    document.getElementById("sumCpuUser").textContent =
      "$" + (totalInfra / sys.activeUsers).toFixed(4);
  }

  // Scaling Section
  document.getElementById("valCurrentInstances").textContent =
    sys.instances + " / " + sys.maxInstances;
  ...
  document.getElementById("mCpuFill").style.background = getUsageColor(
    sys.cpuUsagePct,
  );

  document.getElementById("mRam").textContent = sys.ramUsagePct + "%";
  document.getElementById("mRamFill").style.width = sys.ramUsagePct + "%";
  document.getElementById("mRamFill").style.background = getUsageColor(
    sys.ramUsagePct,
  );
...
async function loadVideoStats() {
  try {
    ...
    if (res.ok) {
      const json = await res.json();
      document.getElementById("kpiTotalVideos").textContent =
        json.data.totalVideos;
      document.getElementById("kpiStorage").textContent =
        json.data.totalStorageUsed;
      document.getElementById("kpiAvgStream").textContent =
        json.data.avgStreamingTime;
      document.getElementById("kpiFailedStream").textContent =
        json.data.failedStreamCount;
    }
  } catch (e) {
...
async function loadMongoDBStats() {
  try {
    ...
    if (res.ok) {
      ...
      document.getElementById("mongoCollections").textContent =
        data.totalCollections.toLocaleString();
      document.getElementById("mongoDocuments").textContent =
      ...
      document.getElementById("mongoStorageSize").textContent =
        data.storageSize;
    } else {
...
async function loadConnectionPool() {
  try {
    ...
    if (res.ok) {
      ...
      stateEl.style.color =
        data.state === "Connected" ? "var(--dev-success)" : "var(--dev-danger)";
      ...
      document.getElementById("mongoNetIn").textContent =
        data.networkBytes.received;
      document.getElementById("mongoNetOut").textContent =
        data.networkBytes.sent;
    } else {
...
async function loadQueryPerformance() {
  try {
    ...
    if (res.ok) {
      ...
      document.getElementById("mongoActiveQueries").textContent =
        data.activeQueries.length;
      document.getElementById("mongoAvgQueryTime").textContent =
        data.avgQueryTime + "ms";
      document.getElementById("mongoInserts").textContent = opcounters.insert
        ? opcounters.insert.toLocaleString()
        : "0";
      document.getElementById("mongoQueries").textContent = opcounters.query
        ? opcounters.query.toLocaleString()
        : "0";
      document.getElementById("mongoUpdates").textContent = opcounters.update
        ? opcounters.update.toLocaleString()
        : "0";
      document.getElementById("mongoDeletes").textContent = opcounters.delete
        ? opcounters.delete.toLocaleString()
        : "0";
      document.getElementById("mongoGets").textContent = opcounters.getmore
        ? opcounters.getmore.toLocaleString()
        : "0";
      document.getElementById("mongoCommands").textContent = opcounters.command
        ? opcounters.command.toLocaleString()
        : "0";
...
async function refreshAtlasMetrics() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      // Update cluster info
      document.getElementById("atlasClusterTier").textContent =
        data.cluster.tier;
      ...
      document.getElementById("atlasNodeCount").textContent =
        data.cluster.nodeCount;
      ...
      if (breakdownEl) {
        ...
        document.getElementById("mongoBaseBreakdown").textContent =
          data.costs.breakdown.base;
        document.getElementById("mongoStorageBreakdown").textContent =
          data.costs.breakdown.storage;
        document.getElementById("mongoTransferBreakdown").textContent =
          data.costs.breakdown.transfer;
        document.getElementById("mongoBackupBreakdown").textContent =
          data.costs.breakdown.backup;
      }

      // Update total cost
      document.getElementById("calcMongoTotal").textContent =
        data.costs.totalMongoCost.toFixed(2);
...
function exportMetricsJSON() {
  ...
  const dataStr =
    ...
    encodeURIComponent(JSON.stringify(activeDashboardData, null, 2));
  const dlAnchorElem = document.createElement("a");
  dlAnchorElem.setAttribute("href", dataStr);
  dlAnchorElem.setAttribute(
    "download",
    `dev-dashboard-metrics-${Date.now()}.json`,
  );
  dlAnchorElem.click();
}
...
function exportMetricsCSV() {
  ...
  const dataString =
    "data:text/csv;charset=utf-8," + encodeURIComponent(csvContent.join("\n"));
