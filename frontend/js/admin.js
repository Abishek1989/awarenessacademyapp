...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 3. Handle Hash Routing & Initial Load
    ...
    async function handleAdminIntroUpload(e) {
        ...
        try {
            ...
            const data = await new Promise((resolve, reject) => {
                ...
                xhr.open('POST', `${Auth.apiBase}/uploads/video/direct`);
            ...
            // Successful Finish
            adminIntroHidden.value = data.fileUrl;

            // Show Preview indicator
            adminVideoPreview.style.display = 'block';
            document.getElementById('adminVideoLink').href = data.fileUrl;
            adminUploadProgress.style.display = 'none';
        } catch (err) {
            console.error(err);
...
function handleHashNavigation() {
    ...
    if (bareHash === 'editCourse') {
        ...
        if (courseId) {
            ...
            fetchAndOpenCourseEdit(courseId);
        } else {
            switchSection('courses', false);
            openCourseModal();
        }
...
async function fetchAndOpenCourseEdit(courseId) {
    try {
...
function switchSection(section, updateHash = true) {
    if (updateHash && section !== 'editCourse') {
...
window.switchAdminCourseTab = function (tabId) {
    // Hide all steps
    const steps = ['adminCourseStep1', 'adminCourseStep2', 'adminCourseStep3', 'adminCourseStep4', 'adminCourseStep5', 'adminCourseStudents'];
    steps.forEach(id => {
        const el = document.getElementById(id);
        if (el) el.style.display = 'none';
    });
    ...
    document.querySelectorAll('.admin-course-tab').forEach(btn => {
        btn.classList.remove('active');
        btn.style.color = '#666';
        btn.style.background = 'transparent';
    });

    // Show target step (students tab uses flex, form steps use block)
    const targetEl = document.getElementById(tabId);
    if (targetEl) targetEl.style.display = tabId === 'adminCourseStudents' ? 'flex' : 'block';

    // Also hide/show the form wrapper itself
    const formEl = document.getElementById('courseForm');
    if (formEl) formEl.style.display = tabId === 'adminCourseStudents' ? 'none' : 'block';

    // Add active styling to clicked tab
    const activeTab = document.querySelector(`.admin-course-tab[data-target="${tabId}"]`);
    if (activeTab) {
    ...
    if (tabId === 'adminCourseStudents') {
        const courseId = document.getElementById('courseId').value;
        if (courseId) {
            loadAdminCourseStudents(courseId);
        } else {
            document.getElementById('adminCourseStudentsContainer').innerHTML =
...
window.loadAdminCourseStudents = async function (courseId) {
    const container = document.getElementById('adminCourseStudentsContainer');
    ...
    try {
        ...
        window.currentAdminCourseStudents = students; // For search filtering
        renderAdminCourseStudents(students);
    } catch (err) {
...
window.renderAdminCourseStudents = function (students) {
    const container = document.getElementById('adminCourseStudentsContainer');
    ...
    students.forEach(enr => {
        ...
        const enrolledDateStr = rawDate ? new Date(rawDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : 'N/A';
        ...
        if (enr.status === 'Active') {
            ...
            statusBadge = '<span class="badge" style="background: rgba(59, 130, 246, 0.1); color: #1D4ED8; padding: 6px 10px; border-radius: 6px; font-weight: 600;">Completed</span>';
        } else {
            statusBadge = '<span class="badge" style="background: rgba(107, 114, 128, 0.1); color: #374151; padding: 6px 10px; border-radius: 6px; font-weight: 600;">Disabled</span>';
        }
            ...
            <div style="background: white; border: 1px solid #f0f0f0; border-radius: 12px; padding: 20px; display: flex; flex-direction: column; gap: 15px; transition: box-shadow 0.3s;" onmouseover="this.style.boxShadow='0 4px 12px rgba(0,0,0,0.05)'" onmouseout="this.style.boxShadow='none'">
                ...
                <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 15px;">
                    ...
                    <div style="display: flex; align-items: center; gap: 15px; flex-wrap: wrap;">
                        ...
                        <div style="display: flex; gap: 8px;">
                            ${btnDisabled
                ? `<button class="btn-primary" style="padding: 6px 12px; font-size: 0.8rem; background: #e6f4ea; color: #1e7e34; border: 1px solid #c3e6cb; border-radius: 6px;" onclick="updateAdminEnrollmentStatus('${enr._id}', 'Active')"><i class="fas fa-check"></i> Enable</button>`
                : `<button class="btn-primary" style="padding: 6px 12px; font-size: 0.8rem; background: #fee; color: #e74c3c; border: 1px solid #f5c6cb; border-radius: 6px;" onclick="updateAdminEnrollmentStatus('${enr._id}', 'Disabled')"><i class="fas fa-ban"></i> Disable</button>`
            }
                            ${enr.courseValidityType === 'Limited' ? `
                                <button class="btn-primary" style="padding: 6px 12px; font-size: 0.8rem; background: #e3f2fd; color: #0d47a1; border: 1px solid #bbdefb; border-radius: 6px;" onclick="promptExtendAdminEnrollment('${enr._id}', '${enr.expiryDate || ''}')" title="Extend Access">
                                    <i class="fas fa-calendar-plus"></i> Extend
                                </button>
                            ` : ''}
                            <button class="btn-primary" style="padding: 6px 12px; font-size: 0.8rem; background: #fee; color: #c0392b; border: 1px solid #e74c3c; border-radius: 6px;" onclick="deleteAdminEnrollment('${enr._id}')" title="Remove Completely">
                ...
                <div style="display: flex; flex-wrap: wrap; gap: 20px; align-items: center; background: #fafafa; padding: 15px; border-radius: 8px;">
                    <div style="flex: 1; min-width: 140px;">
                        ...
                        <div style="font-weight: 500; color: #444; font-size: 0.95rem;">
                            <i class="far fa-calendar-alt" style="color: var(--color-primary); margin-right: 5px;"></i> ${enrolledDateStr}
                        </div>
                    </div>
...
window.currentAdminStudentFilter = 'All';

window.filterAdminStudents = function (statusFilter, btnElement) {
    if (statusFilter && typeof statusFilter === 'string') {
        window.currentAdminStudentFilter = statusFilter;
        if (btnElement) {
    ...
    const status = window.currentAdminStudentFilter || 'All';
    const q = document.getElementById('adminStudentSearch').value.toLowerCase();
    if (!window.currentAdminCourseStudents) return;

    let filtered = window.currentAdminCourseStudents;
    ...
    renderAdminCourseStudents(filtered);
}

window.updateAdminEnrollmentStatus = async function (enrollmentId, newStatus) {
    if (!confirm(`Are you sure you want to change access to ${newStatus}?`)) return;

    // prepare request body; if activating, ask for validity choices
    const body = { status: newStatus };
    if (newStatus === 'Active') {
        const reply = prompt('Enter number of days to grant (leave blank for lifetime access). Cancel to abort:');
        if (reply === null) {
            // user cancelled re-enable
            return;
        }
        const trimmed = reply.trim();
        if (trimmed === '') {
            body.expiryDate = null; // lifetime
        } else {
...
window.deleteAdminEnrollment = async function (enrollmentId) {
...
window.promptExtendAdminEnrollment = async function (enrollmentId, currentExpiry) {
...
async function openCourseModal(course = null) {
    ...
    switchAdminCourseTab('adminCourseStep1'); // Reset to first tab

    // Load Mentors and render Checkboxes (Asynchronous, no await to prevent blocking UI)
    loadMentorsForDropdown(course ? (course.mentors || []) : []);
    ...
    if (course) {
        ...
        // Populate Validity Fields
        const validityType = course.validityType || 'Lifetime';
        if (validityType === 'Lifetime') {
            document.getElementById('validityLifetime').checked = true;
        } else {
            document.getElementById('validityLimited').checked = true;
        }
        document.getElementById('courseValidityDays').value = course.validityDays || '';
        toggleValidityDays();
        ...
        if (document.getElementById('courseWhatsappLink')) {
            document.getElementById('courseWhatsappLink').value = '';
        }
        ...
        // Reset Validity
        document.getElementById('validityLifetime').checked = true;
        document.getElementById('courseValidityDays').value = '';
        toggleValidityDays();
    }

    // Update hash without triggering hashchange event to avoid recursive fetches
    if (course && course._id) {
        history.pushState(null, '', `#editCourse?id=${course._id}`);
    } else {
        history.pushState(null, '', `#editCourse`);
    }

    // Switch to the Course Edit Tab UI
    document.querySelectorAll('.dashboard-section').forEach(s => s.style.display = 'none');
    document.getElementById('adminCourseEditTab').style.display = 'block';

    // Show/hide Students tab — only applicable when editing an existing course
    const stuTab = document.getElementById('adminStudentTabBtn');
    if (course && course._id) {
        stuTab.style.display = 'flex';
        // Load students if clicking on edit existing course
        loadAdminCourseStudents(course._id);
    } else {
        stuTab.style.display = 'none';
    }
}

window.deleteAdminCourse = async function () {
    const courseId = document.getElementById('courseId').value;
    if (!courseId) return;

    // Create a custom red-themed confirmation modal
    const modalHtml = `
        <div id="deleteCourseConfirmModal" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.7); display: flex; align-items: center; justify-content: center; z-index: 9999;">
            <div style="background: white; padding: 30px; border-radius: 12px; max-width: 450px; width: 90%; text-align: center; border-top: 6px solid #dc2626; box-shadow: 0 10px 25px rgba(220, 38, 38, 0.3);">
                ...
                <input type="text" id="confirmCourseIdInput" class="form-control" style="width: 100%; box-sizing: border-box; border: 2px solid #fca5a5; padding: 12px; border-radius: 8px; text-align: center; font-family: monospace; font-size: 1.1rem; margin-bottom: 20px; color: #dc2626; font-weight: bold;" placeholder="Paste ID here" autocomplete="off">
                
                <div style="display: flex; gap: 15px;">
                    <button type="button" onclick="document.getElementById('deleteCourseConfirmModal').remove()" style="flex: 1; padding: 12px; background: #f3f4f6; color: #4b5563; border: 1px solid #d1d5db; border-radius: 8px; font-weight: 600; cursor: pointer; transition: all 0.2s;">Cancel</button>
                    <button type="button" id="finalDeleteCourseBtn" onclick="executeAdminCourseDelete('${courseId}')" disabled style="flex: 1; padding: 12px; background: #dc2626; color: white; border: none; border-radius: 8px; font-weight: 700; cursor: not-allowed; opacity: 0.5; transition: all 0.2s;">Delete Forever</button>
                </div>
            </div>
        </div>
    `;
    ...
    const inputEL = document.getElementById('confirmCourseIdInput');
    const btnEl = document.getElementById('finalDeleteCourseBtn');
...
window.executeAdminCourseDelete = async function (courseId) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (data.message === 'Cannot delete course with active enrollments') {
                ...
                // Switch to students tab automatically so admin can remove them
                switchAdminCourseTab('adminCourseStudents');
            } else {
...
function closeCourseEditTab() {
    document.getElementById('adminCourseEditTab').style.display = 'none';
    switchSection('courses');
}

function toggleValidityDays() {
    const isLimited = document.getElementById('validityLimited').checked;
    const container = document.getElementById('validityDaysContainer');
    if (isLimited) {
        container.style.display = 'block';
        document.getElementById('courseValidityDays').setAttribute('required', 'true');
    } else {
        container.style.display = 'none';
        document.getElementById('courseValidityDays').removeAttribute('required');
    }
}
...
async function saveCourse(e, statusArg) {
    ...
    if (status === 'Published' && selectedMentors.length === 0) {
        ...
        switchAdminCourseTab('adminCourseStep4'); // Switch to Faculty tab
        return;
    }
    ...
    const data = {
        ...
        validityDays: document.getElementById('courseValidityDays').value || 0,
        status: status
    };
    ...
    try {
        ...
        UI.error(error.message || 'Error saving course');
    } finally {
        UI.hideLoader();
    }

    if (networkSuccess) {
        try { closeCourseEditTab(); } catch (e) { console.error('closeCourseEditTab failed', e); }
        loadCourses().catch(e => console.error('loadCourses failed', e));
    }
}
...
async function syncToGoogleSheets() {
    ...
    if (!token) {
        UI.error('Please log in again to sync.');
        return;
    }
    ...
    try {
        ...
        UI.showLoader();
        const response = await fetch(`${Auth.apiBase}/courses/admin/sync-google-sheets`, {
            method: 'POST',
            headers: {
        ...
        UI.success(data.message || 'Successfully synced data to Google Sheets!');
        if (data.syncedCourses) {
            console.log(`Synced ${data.syncedCourses} course(s) to Google Sheets.`);
        }

    } catch (error) {
        console.error('Sync Error:', error);
        UI.error(error.message || 'Failed to sync courses to Google Sheets.');
    } finally {
        UI.hideLoader();
        if (btn) {
            btn.innerHTML = originalText || '<i class="fas fa-sync"></i> Sync Google Sheets';
            btn.disabled = false;
        }
    }
}
