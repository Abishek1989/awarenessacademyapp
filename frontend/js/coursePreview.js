...
async function loadCourseData() {
  try {
    ...
    const [courseRes, modulesRes, assessmentsRes] = await Promise.all([
      fetch(`${Auth.apiBase}/courses/${courseId}`, {
        headers: Auth.getHeaders(),
      }),
      fetch(
        `${Auth.apiBase}/courses/${courseId}/modules?includeUnpublished=true`,
        { headers: Auth.getHeaders() },
      ),
      fetch(`${Auth.apiBase}/exams/course/${courseId}`, {
        headers: Auth.getHeaders(),
      }),
    ]);
...
function renderSidebar() {
  ...
  document.getElementById("progressText").textContent =
    `${modulesData.length} module${modulesData.length !== 1 ? "s" : ""} • ${assessmentsData.length} assessment${assessmentsData.length !== 1 ? "s" : ""}`;

  // Render modules
  let modulesHtml = modulesData
    .map((m, index) => {
      let statusColor = "#ccc";
...
function selectModule(id) {
  ...
  document.getElementById("moduleAuthor").textContent =
    module.createdBy?.name || "Instructor";
  document.getElementById("moduleDate").textContent = new Date(
...
async function toggleCourseStatus(status) {
  if (status === "Pending" && !confirm("Hide this course from students?"))
    return;
  ...
  try {
    ...
    if (res.ok) {
      UI.success(
        `Course ${status === "Approved" ? "Published" : "Unpublished"}`,
      );
      location.reload();
    }
  } catch (err) {
  ...
  if (contentType === "rich-content") {
    ...
    if (module.fileUrl && module.fileMetadata) {
      ...
      try {
        ...
        const { blobUrl } = await DRMProtection.fetchSecureFile(
          module._id,
          module.fileUrl,
        );
      ...
      container.innerHTML =
        '<p style="color: #dc3545;">Video file not found.</p>';
    }
    ...
    if (module.fileUrl && module.fileMetadata) {
      ...
      container.innerHTML =
        '<p style="color: #dc3545;">PDF file not found.</p>';
    }
  }
}
...
async function loadAndDisplayExam(examId) {
  try {
    ...
    if (!res.ok) {
      if (res.status === 404) {
        throw new Error(
          `Assessment not found (ID: ${examId}). It may have been removed or doesn't exist.`,
        );
    ...
    document.getElementById("moduleAuthor").textContent =
      exam.createdBy?.name || "Staff Member";
    document.getElementById("moduleDate").textContent = new Date(
      exam.createdAt,
    ).toLocaleDateString();
...
async function handleExamAction(action, rejectionReason = null) {
  ...
  try {
    ...
    UI.success(
      `Assessment ${action === "Approved" ? "approved" : "rejected"} successfully`,
    );
...
document.addEventListener("click", function (event) {
  if (
    !event.target.closest(".assessment-menu-btn") &&
    !event.target.closest(".assessment-dropdown")
  ) {
    closeAssessmentMenu();
  }
});
